...
func (r *UpdateRequest) toRequestParameter(current *sacloud.PrivateHost) (*sacloud.PrivateHostUpdateRequest, error) {
	req := &sacloud.PrivateHostUpdateRequest{}
...
func (s *Service) Update(req *UpdateRequest) (*sacloud.PrivateHost, error) {
	return s.UpdateWithContext(context.Background(), req)
}

func (s *Service) UpdateWithContext(ctx context.Context, req *UpdateRequest) (*sacloud.PrivateHost, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("reading PrivateHost[%s] failed: %s", req.ID, err)
	}
