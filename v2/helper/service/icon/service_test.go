...
func TestIconService_CRUD(t *testing.T) {
	...
	testutil.RunResource(t, &testutil.ResourceTestCase{
		...
		Tests: []testutil.ResourceTestFunc{
			...
			// delete
			func(ctx context.Context, caller sacloud.APICaller) error {
				return svc.Delete(&DeleteRequest{ID: icon.ID})
			},
		},
		Cleanup:  testutil.ComposeCleanupResourceFunc(prefix, testutil.CleanupTargets.Icon),
		Parallel: true,
	})
}
