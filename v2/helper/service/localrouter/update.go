...
func (r *UpdateRequest) toRequestParameter(current *sacloud.LocalRouter) (*sacloud.LocalRouterUpdateRequest, error) {
	req := &sacloud.LocalRouterUpdateRequest{}
...
func (s *Service) UpdateWithContext(ctx context.Context, req *UpdateRequest) (*sacloud.LocalRouter, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("reading the local router resource[%s] failed: %s", req.ID, err)
	}
