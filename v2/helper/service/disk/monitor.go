...
func (s *Service) Monitor(req *MonitorRequest) ([]*sacloud.MonitorDiskValue, error) {
	return s.MonitorDiskWithContext(context.Background(), req)
}

func (s *Service) MonitorDiskWithContext(ctx context.Context, req *MonitorRequest) ([]*sacloud.MonitorDiskValue, error) {
	...
	values, err := client.Monitor(ctx, req.Zone, req.ID, cond)
	if err != nil {
		return nil, err
	}
