...
func (r *UpdateRequest) toRequestParameter(current *sacloud.NFS) (*sacloud.NFSUpdateRequest, error) {
	req := &sacloud.NFSUpdateRequest{}
...
func (s *Service) UpdateWithContext(ctx context.Context, req *UpdateRequest) (*sacloud.NFS, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("reading NFS[%s] failed: %s", req.ID, err)
	}
