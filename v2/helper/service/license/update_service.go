...
func (s *Service) Update(req *UpdateRequest) (*sacloud.License, error) {
	return s.UpdateWithContext(context.Background(), req)
}

func (s *Service) UpdateWithContext(ctx context.Context, req *UpdateRequest) (*sacloud.License, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("reading License[%s] failed: %s", req.ID, err)
	}
