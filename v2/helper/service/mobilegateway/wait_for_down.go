...
func (s *Service) WaitForDown(req *WaitForDownRequest) (*sacloud.MobileGateway, error) {
	return s.WaitForDownWithContext(context.Background(), req)
}

func (s *Service) WaitForDownWithContext(ctx context.Context, req *WaitForDownRequest) (*sacloud.MobileGateway, error) {
	if err := req.Validate(); err != nil {
		return nil, err
	}
	client := sacloud.NewMobileGatewayOp(s.caller)
	return wait.UntilMobileGatewayIsDown(ctx, client, req.Zone, req.ID)
}
