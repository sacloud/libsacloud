...
func (r *UpdateRequest) toRequestParameter(current *sacloud.MobileGateway) (*sacloud.MobileGatewayUpdateRequest, error) {
	req := &sacloud.MobileGatewayUpdateRequest{}
...
func (s *Service) UpdateWithContext(ctx context.Context, req *UpdateRequest) (*sacloud.MobileGateway, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("reading MobileGateway[%s] failed: %s", req.ID, err)
	}
